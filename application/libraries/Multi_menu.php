...
class Multi_menu {
	 ...
	 * @var string
	 */
	private $parent_anchor            = '<a href="%s" class="dropdown-toggle" data-toggle="dropdown">%s</a>';
    ...
    public function render($config = array(), $divided_items_list = array(), $divider = '')
    {
    	...
    	if ( count($this->items) ) 
    	{
	        ...
	        $this->render_herci($items, $html);
    	}

        return $html;
    }
    
    public function render_basic($config = array(), $divided_items_list = array(), $divider = '')
    {
		$html  = "";
     ...
     * By hercival Aragon
     */
    public function raw_data($config = array(), $divided_items_list = array(), $divider = '')
    {
		$menudata  = array();
     ...
     * @return array         
     */
    private function prepare_items(array $data, $parent = 0)
    {
    	$items = array();
    ...
    private function render_herci($items, &$html = '', $parent_alias = '')
	{	    
	   ...
	   $html .= '<ul class="nav navbar-nav">'; 	
	    ...
	    foreach ($items as $item)
	    {
	        ...
	        if ( isset($item[$this->menu_label], $item[$this->menu_key]) ) 
	        {
		        ...
		        // menu slug
                if($parent_alias){
                    $slug  = $parent_alias.'/'.$item[$this->menu_key];
                }else{
                    $slug  = $item[$this->menu_key];
                }
		        ...
		        if ($has_children){   
                    $tag_open = '<li class="dropdown menu-large">';
		        ...
		        if( $has_children ){	        	
		            $this->render_herci2($item['children'], $html,$slug);
				    $html  .= '</li>';
		        }
		        else {
		        	$html .= '</li>'; 
    ...
    private function render_herci2($items, &$html = '', $parent_alias = '')
	{	    

	   $html .= '<ul class="dropdown-menu megamenu row" style="font-size: 12px;">'; 	
	    ...
	    foreach ($items as $item)
	    {
	        ...
	        if ( isset($item[$this->menu_label], $item[$this->menu_key]) ) 
	        {
		        ...
		        if ($has_children){   
                    $tag_open = '<li class="col-sm-2" style="width:20%;">
                                    <ul>';
                    
                    if($menu_type == 'heading'){
                        $item_anchor = '<li class="dropdown-header" style="padding:0;"><span>%s</span></li>';
                    }else{
                        $item_anchor = '<li class="dropdown-header" style="padding:0;"><a href="%s" >%s</a></li>';
                    }
		        ...
		        if( $has_children ){	        	
		            $this->render_herci3($item['children'], $html,$slug);
				    $html  .= '</ul></li>';
		        }
		        else {
		        	$html .= '</li>'; 
    ...
    private function render_herci3($items, &$html = '', $parent_alias = '')
	{	    
    ...
    private function basic($items, &$html = '', $parent_alias = '')
	{	    
     ...
     * By hercival Aragon
     */
    private function raw_item($items, &$menudata = array())
	{	    
	    ...
	    foreach ($items as $item)
	    {
	        ...
	        if ( isset($item[$this->menu_label], $item[$this->menu_key]) ) 
	        {
		        $id = $item[$this->menu_id];
                $title = $item[$this->menu_label];
		        $alias  = $item[$this->menu_key];
                $parent  = $item[$this->menu_parent];
                $menu_order  = $item[$this->menu_order];
                ...
                array_push($menudata, $current);
                // has children or not
		        $has_children = ! empty($item['children']);	        
		        if ( $has_children ) 
		        {	        	
		            $this->raw_item($item['children'], $menudata);	            
		        }
	        }
	    }
	}
